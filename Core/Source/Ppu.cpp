...
void Ppu::PixelTransfer()
{
	...
	uint8_t byte1 = m_Bus->ReadBus(tile_address + (tile_id * 16) + (m_Display->context.ly % 16));
	uint8_t byte2 = m_Bus->ReadBus(tile_address + (tile_id * 16) + (m_Display->context.ly % 16) + 1);

	const unsigned long tile_colours[4] = { 0xFFFFFFFF, 0xFFAAAAAA, 0xFF555555, 0xFF000000 };
	uint8_t high = (static_cast<bool>(byte1 & (1 << (m_PixelX % 8)))) << 1;
	uint8_t low = (static_cast<bool>(byte2 & (1 << (m_PixelX % 8)))) << 0;
